...
router.post('/register', async (req, res) => {
  ...
  if (studentInDB) {
    if (studentInDB.isVerified) return res.status(400).send('Account already exists. Please log in.');
    else return res.status(200).send('TODO: Redirect to token verification');
  }
  ...
  try {
    ...
    mailer.sendVerificationMail(email, "Verify your email ID", `${config.get("domain")}/verify.html?token=${token}`);
...
router.post('/changepwd', tokenAuth, async (req, res) => {
  ...
  const { error } = Joi.validate({ password: newPassword }, { password: Joi.string().required().min(8).max(255) });
  if (error) return res.status(400).send(error.details[0].message);
  // Update new password
